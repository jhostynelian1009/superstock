        ...
        <form action="{{ route('admin.categorias.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="min-h-[120px] w-full resize-y rounded-sm border border-[#e3e3e0] bg-white px-4 py-3 text-sm text-[#1b1b18] focus:border-[#F53003] focus:outline-none focus:ring-2 focus:ring-[rgba(245,48,3,0.20)] dark:border-[#3E3E3A] dark:bg-[#161615] dark:text-[#EDEDEC]">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-[13px] text-[#F53003]">{{ $message }}</p>
                @enderror
            </div>
