...
@section('content')
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-[#706f6c] dark:text-[#A1A09A]">Actualiza la información del usuario.</p>
    </div>
    ...
    <div class="max-w-2xl rounded-lg bg-white p-6 shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:bg-[#161615] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d]">
        <form action="{{ route('admin.usuarios.update', $usuario) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $usuario->email) }}" required maxlength="255"
            ...
            <div class="rounded-lg bg-[#fff2f2] p-4 dark:bg-[#1D0002]">
                <p class="mb-3 text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">Cambiar contraseña (opcional)</p>
                
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        <label for="password" class="mb-2 block text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">Nueva contraseña</label>
                        ...
                        <p class="mt-1 text-xs text-[#706f6c] dark:text-[#A1A09A]">Mínimo 8 caracteres. Déjalo vacío para mantener la contraseña actual.</p>
                        @error('password')
                            <p class="mt-1 text-[13px] text-[#F53003]">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="password_confirmation" class="mb-2 block text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">Confirmar contraseña</label>
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_active" id="is_active" value="1" @checked(old('is_active', $usuario->is_active))
